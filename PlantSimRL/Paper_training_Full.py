...
pfad = 'D:\\Studium\Projekt\Paper\PlantSimRL\simulations'
erg = 'ergebnisse_Test4\\'
mod = 'models_Test4\\'  # _V1
...
for i in range(sim_count):  # sim_count
    ...
    # Eval von 5 Plänen die nicht teil vom Training sind & Eval Rand actions
    open(Training['Logs'][i] + '_LSTM' + '\\' + Training['Logname'][i] + '_Testing.txt', "w")
    for j in range(sim_count):
        ...
        while not done:
            ...
            episode_starts = done
            reward_sum += rewards
        with open(Training['Logs'][i] + '_LSTM' + '\\' + Training['Logname'][i] + '_Testing.txt', "a") as datei:
            # Die Werte in die Datei schreiben, einen pro Zeile
            datei.write('Steps = ' + str(steps) + "\n")
            datei.write('Return = ' + str(reward_sum) + "\n")
            datei.write('Info = ' + str(info) + "\n")
    # Eval von Random Actions
    open(Training['Logs'][i] + '_LSTM' + '\\' + Training['Logname'][i] + '_Random.txt', "w")
    for j in range(5):
        done = False
        reward_sum = 0
        steps = 0
        ...
        while not done:
            ...
            obs, rewards, done, info = env.step(action)
            reward_sum += rewards
        with open(Training['Logs'][i] + '_LSTM' + '\\' + Training['Logname'][i] + '_Random.txt', "a") as datei:
            # Die Werte in die Datei schreiben, einen pro Zeile
            datei.write('Steps = ' + str(steps) + "\n")
            datei.write('Return = ' + str(reward_sum) + "\n")
            datei.write('Info = ' + str(info) + "\n")
